...
def case1(index=CASE_1_ATTRIBUTE_INDEX,output=True,ret='accuracy'):
    ...
    for i in xrange(NUMBER_OF_ROUNDS):
        ...
        for row in test_set:
            ...
            posterior_spam = prior_spam * stats.cauchy(sample_mean_word_spam, sample_std_dev_spam).pdf(row[index])

            posterior_ham = prior_ham * stats.cauchy(sample_mean_word_ham, sample_std_dev_ham).pdf(row[index])
    ...
    if output:
        print "\033[1;32m"
        print '============================================='
        print 'CASE 1 - ONE ATTRIBUTE - USING CAUCHY MODEL'
        print '============================================='
        print "\033[00m"
