...
def case3():
    ...
    for i in xrange(NUMBER_OF_ROUNDS):
        ...
        for j in list(range(54)):
            ...
            p_char_spam = nb.take_p_char_spam(train_set,j,SPAM_ATTR_INDEX)
            p_char = nb.take_p_attribute(train_set,j,SPAM_ATTR_INDEX)
        ...
        for row in test_set:
            ...
            for j in list(range(10)):
                ...
                if(nb.attribute_not_in_row(row,j)):
                    if(row[SPAM_ATTR_INDEX] != guess):
                        hits += (1/54)
                    else:
                        misses += (1/54)
                elif(nb.attribute_in_row(row,j)):
                    if(row[SPAM_ATTR_INDEX] == guess):
                        hits += (1/54)
                    else:
                        misses += (1/54)
