...
def case2():
    ...
    for i in xrange(NUMBER_OF_ROUNDS):
        ...
        for row in test_set:
            ...
            product_of_all_conditional_probs_spam = reduce(lambda acc,cur: acc * stats.chi(sample_means_word_spam[cur], sample_std_devs_spam[cur]).pdf(row[CASE_2_ATTRIBUTE_INDEXES[cur]]) , xrange(10), 1)
            ...
            product_of_all_conditional_probs_ham = reduce(lambda acc,cur: acc * stats.chi(sample_means_word_ham[cur], sample_std_devs_ham[cur]).pdf(row[CASE_2_ATTRIBUTE_INDEXES[cur]]) , xrange(10), 1)
    ...
    print ''
    print 'CASE 2 - TEN ATTRIBUTES - USING CHI MODEL'
