...
def case2(indexes=CASE_2_ATTRIBUTE_INDEXES,output=True):
    ...
    for i in xrange(NUMBER_OF_ROUNDS):
        ...
        ps_attribute_spam = map(lambda elem: nb.take_p_attribute_spam(train_set,indexes[elem],SPAM_ATTR_INDEX),xrange(10))
        ps_attribute_ham = map(lambda elem: nb.take_p_attribute_ham(train_set,indexes[elem],SPAM_ATTR_INDEX),xrange(10))
        ...
        for row in test_set:
            ...
            product_p_spam_attributes = reduce(lambda acc,elem: acc * ps_attribute_spam[elem] if row[indexes[elem]] == 1 else acc, xrange(10),1.0)
            product_p_ham_attributes = reduce(lambda acc,elem: acc * ps_attribute_ham[elem] if row[indexes[elem]] == 1 else acc, xrange(10),1.0)
